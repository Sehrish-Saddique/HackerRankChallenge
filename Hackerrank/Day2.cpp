...
void solve(double meal_cost, int tip_percent, int tax_percent) {
  double tip =(meal_cost/100)*tip_percent; // cout<<"Total tip Meal Cost is: "<<  (tip );

 double tax =(meal_cost/100)*tax_percent ; // cout<<"Total tax  Meal Cost is: "<<  (tax );

 double cost=0;
 cost=tip +tax +meal_cost;
  cout<<  round(cost);
